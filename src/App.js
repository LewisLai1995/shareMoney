...
function SpinnerWheel({ names, rem=1, onDone }) {
  ...
  const spin = () => {
    ...
    winnerRef.current = names[winIdx];

    const sliceAngle2 = 360 / names.length;
    const sliceOffset = (0.15 + Math.random() * 0.7) * sliceAngle2;
    const targetAngle = winIdx * sliceAngle2 + sliceOffset;
    const extraRounds = (8 + Math.floor(Math.random() * 5)) * 360;
    const finalAngle  = extraRounds + (360 - targetAngle);

    setTarget(finalAngle);
    setAngle(0);
    setPhase("resetting");  // Phase 1: snap to 0 with no transition
  };

  // Phase 2: once reset renders, start the spin
  useEffect(() => {
    if(phase === "resetting") {
      const t = setTimeout(() => {
        setAngle(target);
        setPhase("spinning");
      }, 60);  // 60ms gives browser time to paint angle=0
  ...
  const isSpinning = phase === "resetting" || phase === "spinning";
  const showTransition = phase === "spinning" || phase === "done";
  ...
  return (
    <Modal onClose={()=>phase==="idle"||phase==="done" ? onDone(winner) : null}>
      <div style={{ textAlign:"center" }}>
        ...
        <div style={{ position:"relative",width:200,height:200,margin:"0 auto 16px" }}>
          <svg width={200} height={200}
            style={{ transform:`rotate(${angle}deg)`,
              transition: showTransition ? "transform 2.5s cubic-bezier(.17,.67,.12,1)" : "none" }}>
            {names.map((n,i)=>{
              const a1=(i*sliceAngle-90)*Math.PI/180, a2=((i+1)*sliceAngle-90)*Math.PI/180;
              const x1=100+90*Math.cos(a1),y1=100+90*Math.sin(a1);
              const x2=100+90*Math.cos(a2),y2=100+90*Math.sin(a2);
              const lx=100+55*Math.cos((a1+a2)/2),ly=100+55*Math.sin((a1+a2)/2);
              return (
                <g key={i}>
                  <path d={`M100,100 L${x1},${y1} A90,90 0 0,1 ${x2},${y2} Z`}
                    fill={MEMBER_COLORS[i%MEMBER_COLORS.length]} opacity={.85}/>
                  <text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
                    fontSize={names.length>4?9:11} fontWeight="700" fill="#fff"
                    fontFamily="'Noto Sans TC',sans-serif">{n.slice(0,3)}</text>
                </g>
              );
            })}
          </svg>
        ...
        {phase==="idle" && (
          <div style={{ fontSize:13,color:"#64748B",marginBottom:12 }}>點下方按鈕開始</div>
        )}
        {isSpinning && (
          <div style={{ fontSize:13,color:"#64748B",marginBottom:12 }}>轉動中…</div>
        )}
        <button onClick={phase==="done" ? ()=>onDone(winner) : spin}
          disabled={isSpinning}
          style={{ ...actionBtn,
            background: phase==="done" ? "#16A34A" : "#2563EB",
            color:"#fff", width:"100%", opacity: isSpinning ? .5 : 1 }}>
          {phase==="done" ? "確認" : isSpinning ? "轉動中…" : "開始轉！"}
        </button>
      </div>
    </Modal>
  );
...
function BookApp({ bookId, currentUser, userProfile, onBack, onOpenSettings }) {
  ...
  const datesWithData = [...new Set(expenses.filter(e=>!e.isSettlement).map(e=>e.date))];
  const datesFlagged  = [...new Set(expenses.filter(e=>!e.isSettlement&&Object.keys(e.flags||{}).length>0).map(e=>e.date))];
  const totalTWD = expenses.filter(e=>!e.isSettlement).reduce((s,e)=>s+toTWD(e.amount,e.currency),0);
  ...
  return (
    <div onTouchStart={e=>{ swipeTouchX.current=e.touches[0].clientX; }} onTouchEnd={e=>{ const dx=e.changedTouches[0].clientX-(swipeTouchX.current||0); if(dx>window.innerWidth*0.72){ onBack(); } swipeTouchX.current=null; }} style={{ fontFamily:"'Noto Sans TC','PingFang TC',sans-serif",background:"#F0F7FF",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:"calc(80px + env(safe-area-inset-bottom, 0px))" }}>
      ...
      {/* Header */}
      <div style={{ width:"100%",maxWidth:540,background:bookColor,padding:"env(safe-area-inset-top, 0px) 16px 10px",borderRadius:"0 0 16px 16px",boxShadow:`0 8px 28px ${bookColor}44` }}>
...
function SettingsPanel({ currentUser, userProfile, members, bookId, onClose, onSwitchUser, bookColor="#2563EB" }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      localStorage.setItem("splitpay_profile", JSON.stringify({ emoji:selEmoji, nickname:nickname.trim() }));
      onClose();
    } catch(e){ alert("儲存失敗"); }
    setSaving(false);
  };
...
function HomeScreen({ currentUser, onEnterBook }) {
  ...
  const [showInstallHome, setShowInstallHome] = useState(false);
  ...
  return (
    <>
    <div style={{ minHeight:"100vh",background:"#F0F7FF",fontFamily:"'Noto Sans TC',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",padding:"32px 16px 24px",minHeight:"100vh" }}>
    ...
    {showInstallHome&&(
      <Modal onClose={()=>setShowInstallHome(false)}>
        <div>
          <div style={{ fontWeight:800,fontSize:17,color:"#1E3A5F",marginBottom:4 }}>📲 建立捷徑教學</div>
          <div style={{ fontSize:12,color:"#94A3B8",marginBottom:16 }}>把這個 App 加到主畫面，用起來像真正的 App！</div>
          <div style={{ background:"#F0F7FF",borderRadius:12,padding:14,marginBottom:10 }}>
            <div style={{ fontWeight:700,fontSize:14,color:"#1D4ED8",marginBottom:8 }}>🍎 iPhone（Safari）</div>
            <div style={{ fontSize:13,color:"#334155",lineHeight:1.8 }}>1. 用 <b>Safari</b> 開啟此網址<br/>2. 點下方 <b>分享按鈕 ⬆️</b><br/>3. 選「<b>加入主畫面</b>」<br/>4. 點右上角「<b>新增</b>」</div>
          </div>
          <div style={{ background:"#F0FDF4",borderRadius:12,padding:14,marginBottom:16 }}>
            <div style={{ fontWeight:700,fontSize:14,color:"#16A34A",marginBottom:8 }}>🤖 Android（Chrome）</div>
            <div style={{ fontSize:13,color:"#334155",lineHeight:1.8 }}>1. 用 <b>Chrome</b> 開啟此網址<br/>2. 點右上角 <b>⋮ 選單</b><br/>3. 選「<b>加入主畫面</b>」</div>
          </div>
          <button onClick={()=>setShowInstallHome(false)} style={{ ...actionBtn,background:"#2563EB",color:"#fff",width:"100%" }}>GOT IT! 👍</button>
        </div>
      </Modal>
    )}
    {/* Archived books page */}
    {showArchived&&(
      <div style={{ position:"fixed",inset:0,background:"#F0F7FF",zIndex:500,fontFamily:"'Noto Sans TC',sans-serif",overflowY:"auto" }}>
        <div style={{ background:"linear-gradient(135deg,#64748B,#94A3B8)",padding:"calc(env(safe-area-inset-top,0px)+14px) 18px 14px",borderRadius:"0 0 18px 18px" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
...
function LoginScreen({ onLogin, pendingJoin }) {
  ...
  const handleRegister = async () => {
    const users = JSON.parse(localStorage.getItem("splitpay_users")||"{}");
    users[name.trim()] = { emoji:selEmoji };
      ...
      <div style={{ background:"#fff",borderRadius:20,padding:24,width:"100%",maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,.25)" }}>
        ...
        {step==="setup"&&(
          <>
            ...
            <div style={{ fontSize:13,color:"#94A3B8",marginBottom:12 }}>選個你的頭像 Emoji</div>
            ...
            <div style={{ display:"flex",gap:8 }}>
              <button onClick={()=>setStep("name")} style={{ ...actionBtn,background:"#F1F5F9",color:"#64748B",flex:1 }}>返回</button>
              <button onClick={handleRegister} style={{ ...actionBtn,background:"#2563EB",color:"#fff",flex:2 }}>完成</button>
            </div>
          </>
        )}
