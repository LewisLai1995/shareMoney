    ...
    <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
      ...
      {showPicker && (
        <div style={{ overflow:"hidden", borderRadius:9, border:"1.5px solid #BFDBFE", background:"#F8FBFF", marginBottom:6 }}>
          <input type="date" defaultValue={selected} autoFocus
            onChange={e=>e.target.value&&jumpTo(e.target.value)}
            style={{ width:"100%", padding:"7px 10px", border:"none", fontSize:13, fontFamily:"inherit",
              boxSizing:"border-box", background:"transparent", color:"#1E3A5F",
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1D4ED8,#3B82F6)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'Noto Sans TC','PingFang TC',sans-serif" }}>
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:24, width:"100%", maxWidth:340, boxShadow:"0 20px 60px rgba(0,0,0,0.25)" }}>
        {members.length > 0 && !isNew ? (
          ...
          <>
            <div style={{ fontWeight:800, fontSize:16, color:"#1E3A5F", marginBottom:14 }}>輸入你的暱稱</div>
              ...
              <div style={{ display:"flex", justifyContent:"center", gap:6, marginTop:8 }}>
                {Array.from({length:totPages},(_,i)=>(
                  <button key={i} onClick={()=>setPage(i+1)} style={{
                    ...
                    background:page===i+1?bookColor:"#EFF6FF", color:page===i+1?"#fff":bookColor,
              ...
              {members.map(m=>{
                ...
                return (
                  <div key={m.name} style={{ display:"flex", alignItems:"center", gap:9, marginBottom:9 }}>
                    ...
                    <div style={{ fontWeight:800, fontSize:15, color:b>0.01?"#10B981":b<-0.01?"#EF4444":"#94A3B8" }}>
                      {Math.round(b)===0?"±0":`NT${b>0?"+":""}${Math.round(b)}`}
                    </div>
                  </div>
                );
              })}
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:9, padding:"11px 13px" }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:800, fontSize:14, color:"#1E3A5F" }}>
                        ...
                        {m.hasPlusOne&&<span style={{ fontSize:11, color:"#94A3B8", marginLeft:5 }}>👫 有攜伴</span>}
                      </div>
                      <div style={{ fontSize:10, color:"#94A3B8", marginTop:1 }}>
                        已付 NT${paid.toLocaleString()} · {appInfo?`${appInfo.emoji} ${appInfo.label}`:"未設定付款方式"}
                      </div>
                    </div>
                    <button onClick={()=>setEditMember(isEditing?null:{...m})}
                      style={{ ...actionBtn, background:isEditing?"#EFF6FF":bookColor, color:isEditing?bookColor:"#fff", fontSize:11, padding:"6px 11px" }}>
                      {isEditing?"關閉":"設定"}
                    </button>
                    ...
                    <div style={{ borderTop:"1px solid #EFF6FF", padding:"12px 13px", background:"#F8FBFF" }}>
                      ...
                      <div style={{ marginBottom:10 }}>
                        ...
                        <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginBottom:7 }}>
                          {PAYMENT_APPS.map(a=>(
                            <button key={a.id} onClick={()=>setEditMember(p=>({...p,paymentApp:a.id}))} style={{
                              border:"1.5px solid #BFDBFE", borderRadius:20, padding:"4px 9px", cursor:"pointer",
                              fontSize:11, fontWeight:700, fontFamily:"inherit",
                              background:editMember.paymentApp===a.id?bookColor:"#fff",
                              color:editMember.paymentApp===a.id?"#fff":"#334155"
...
const actionBtn = {
  ...
  cursor:"pointer", fontWeight:800, fontSize:14, fontFamily:"inherit", transition:"all .15s"
};
const wkBtn = {
  fontSize:10, fontWeight:700, background:"#EFF6FF", color:"#2563EB",
  border:"none", borderRadius:7, padding:"3px 8px", cursor:"pointer", fontFamily:"inherit"
};
const arrowBtn = {
  background:"none", border:"none", cursor:"pointer", color:"#94A3B8",
  fontSize:16, padding:"0 3px", lineHeight:1, flexShrink:0
};
const pgBtn = {
  background:"#EFF6FF", border:"none", borderRadius:7, width:26, height:26,
  cursor:"pointer", color:"#2563EB", fontSize:15, fontWeight:700, fontFamily:"inherit",
  display:"flex", alignItems:"center", justifyContent:"center"
};
