...
function BookApp({ bookId, currentUser, userProfile, onBack, onOpenSettings }) {
  ...
  const myFlags = expenses.filter(e=>e.flags&&e.flags[currentUser]&&e.createdBy!==currentUser).length;
        ...
        <Modal onClose={()=>setShowInvite(false)}>
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ background:"#EFF6FF",borderRadius:12,padding:14,marginBottom:12 }}>
              ...
              <button onClick={()=>{ navigator.clipboard?.writeText(inviteLink); showToast("📋 已複製"); setShowInvite(false); }} style={{ ...actionBtn,background:bookColor,color:"#fff",width:"100%",fontSize:12 }}>複製連結</button>
            </div>
...
function JoinScreen({ bookId, currentUser, onDone }) {
  ...
  const join = async (chosenName) => {
    ...
    if(!existing){
      await addDoc(collection(db,"books",bookId,"members"),{ name:finalName,hasPlusOne:false,paymentApp:null,paymentCustomLabel:"",paymentCustomUrl:"",createdAt:serverTimestamp() });
    }
  ...
  return (
    <div style={{ minHeight:"100vh",background:`linear-gradient(135deg,${bg}dd,${bg})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Noto Sans TC',sans-serif" }}>
      ...
      <div style={{ color:"rgba(255,255,255,.7)",fontSize:13,marginBottom:28 }}>{members.length} 位成員 · 你被邀請加入</div>
